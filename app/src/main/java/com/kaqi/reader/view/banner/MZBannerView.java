package com.kaqi.reader.view.banner;
...
import com.kaqi.reader.view.banner.holder.MZViewHolder;
 ...
 * modified by forthorn on 2017/08/15
 */
...
public class MZBannerView<T> extends RelativeLayout {
    ...
    private int mDelayedTime = 10000;// Banner 切换时间间隔
    private ViewPagerScroller mViewPagerScroller;//控制ViewPager滑动速度的Scroller
    private boolean mIsOpenMZEffect = false;// 开启魅族Banner效果
    ...
    private int[] mIndicatorRes = new int[]{R.drawable.shape_dots_select, R.drawable.shape_dots_default};
    ...
    private void readAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(R.styleable.MZBannerView);
        mIsOpenMZEffect = typedArray.getBoolean(R.styleable.MZBannerView_open_mz_mode, false);
...
//        }
        ...
        mViewPager = (ScaleViewPager) view.findViewById(R.id.mzbanner_vp);
...
//            mViewPager.setPageTransformer(false, new CustomTransformer());
        ...
        for (int i = 0; i < mDatas.size(); i++) {
            ...
            if (mIndicatorAlign == IndicatorAlign.LEFT.ordinal()) {
                if (i == 0) {
                    ...
                    imageView.setPadding(paddingLeft + 10, 0, 10, 0);
                } else {
                ...
                if (i == mDatas.size() - 1) {
                    ...
                    imageView.setPadding(10, 0, 10 + paddingRight, 0);
                } else {
                    imageView.setPadding(10, 0, 10, 0);
                }
    ...
    public void setPages(List<T> datas, MZHolderCreator mzHolderCreator) {
        ...
        //add by forthorn  已在ScaleViewPager中进行处理，不需要再额外处理
//        setOpenMZEffect();
        ...
        mViewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                int realSelectPosition = mCurrentItem % size;
    ...
    private void adjustZposition(int z) {
        Map<Integer, View> pages = mAdapter.getPages();
        for (Integer integer : pages.keySet()) {
            if (integer == mViewPager.getCurrentItem()) {
                //4.4系统适配
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    pages.get(integer).animate().z(z);
                }
                break;
            }
        }
    ...
    private void resetZposition(int z) {
        Map<Integer, View> pages = mAdapter.getPages();
        for (Integer integer : pages.keySet()) {
            if (integer == mCurrentItem) {
                pages.get(integer).animate().z(z);
            }
        }
    }
...
//            int currentItem = canLoop ? getStartSelectItem() : 0;
            int currentItem = canLoop ? getStartSelectItem() : 2;
            //设置当前选中的Item
        ...
        public Map<Integer, View> getPages() {
            return pages;
        }
