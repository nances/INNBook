...
/**
 * Created by Niqiao on 2019年06月30日00:30:28.
 * 对文字操作的工具类
 */
...
//        ConversionType currentConversionType = ConversionType.S2TWP;
        int convertType = SharedPreUtils.getInstance().getInt(ReadSettingManager.SHARED_READ_CONVERT_TYPE, 0);
...
//
    ...
    public static String formatBookSum(int now) {
        long now_pattern = 10000;
        String units = "万";
        if (now < 10000) {
            now_pattern = 1;
            units = "";
        } else if (now < 100000000) {
            now_pattern = 10000;
            units = "万";
        } else {
            now_pattern = 100000000;
            units = "亿";
        }
        DecimalFormat df = new DecimalFormat("0.000");//设置保留位数
        BigDecimal bg_now = new BigDecimal(df.format(((float) now / now_pattern)));
        String f_now = bg_now.setScale(2, BigDecimal.ROUND_UP).doubleValue() + "";
        if (units.equals("") && f_now.indexOf(".") > 0) {
            f_now = f_now.replaceAll("0+?$", "");//去掉多余的0
            f_now = f_now.replaceAll("[.]$", "");//如最后一位是.则去掉
        }
        return f_now + units;
    }
}
